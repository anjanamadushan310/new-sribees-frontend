...
const Analytics: React.FC = () => {
    ...
    const handleExportKpiCSV = () => {
        ...
        downloadCSV(headers, rows, `analytics_kpis_${slt().format('YYYYMMDD')}.csv`);
    };
    ...
    const handleExportSalesTrendCSV = () => {
        ...
        downloadCSV(headers, rows, `sales_trend_chart_data_${slt().format('YYYYMMDD')}.csv`);
    };
    ...
    const handleExportCategoriesCSV = () => {
        ...
        downloadCSV(headers, rows, `category_share_chart_data_${slt().format('YYYYMMDD')}.csv`);
    };
    ...
    const handleExportOrderStatusCSV = () => {
        ...
        downloadCSV(headers, rows, `order_fulfilment_data_${slt().format('YYYYMMDD')}.csv`);
    };
    ...
    const handleExportProductsCSV = () => {
        ...
        downloadCSV(headers, rows, `top_products_${slt().format('YYYYMMDD')}.csv`);
    };
    ...
    const handleExportCustomersCSV = () => {
        ...
        const rows = data.map((c, idx) => [
            ...
            c.orders,
            c.revenue,
            c.last_order_at ? slt(c.last_order_at).format('YYYY-MM-DD') : ''
        ]);
        downloadCSV(headers, rows, `top_customers_${slt().format('YYYYMMDD')}.csv`);
    };
    ...
    const handleExportBranchesCSV = () => {
        ...
        downloadCSV(headers, rows, `branch_performance_${slt().format('YYYYMMDD')}.csv`);
    };
    ...
    const customerColumns: ColumnsType<TopCustomer> = [
        ...
        },
        {
            ...
            render: (v: string | null) =>
                ...
                v ? slt(v.slice(0, 10)).format('MMM D, YYYY') : <Text type="secondary">—</Text>,
        },
    ];
            ...
            <div className="print-header" style={{ display: 'none' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div style={{ textAlign: 'right', fontSize: 12, color: '#666' }}>
                        <div><strong>Report Period:</strong> {days} Days ({summary ? `${slt(summary.range.start_date).format('MMM D, YYYY')} - ${slt(summary.range.end_date).format('MMM D, YYYY')}` : ''})</div>
                        ...
                        <div><strong>Generated At:</strong> {slt().format('YYYY-MM-DD HH:mm:ss')}</div>
                    </div>
                </div>
            </div>
                ...
                <div>
                    ...
                    {summary && (
                        <Text type="secondary" style={{ fontSize: 12 }} className="no-print">
                            ...
                            {' · compared with '}
                            {slt(summary.range.previous_start_date).format('MMM D')} –{' '}
                            {slt(summary.range.previous_end_date).format('MMM D')}
                    ...
                    >
                        <div>
                            ...
                            <div style={{ fontSize: '12px', color: '#cbd5e1', marginTop: '4px' }}>
                                Period: {days} Days ({summary ? `${slt(summary.range.start_date).format('MMM D, YYYY')} – ${slt(summary.range.end_date).format('MMM D, YYYY')}` : ''})
                            </div>
                        </div>
                        <div style={{ textAlign: 'right', fontSize: '11px', color: '#94a3b8', lineHeight: 1.6, borderLeft: '1px solid #334155', paddingLeft: '16px' }}>
                            ...
                            <div><strong>Generated At:</strong> <span style={{ color: '#f8fafc' }}>{slt().format('YYYY-MM-DD HH:mm')}</span></div>
