        ...
        <div data-testid="analytics-page">
            ...
            >
                ...
                >
                    ...
                    >
                        <div>
                            <h1 style={{ margin: 0, color: '#1890ff', fontSize: 26, fontWeight: 700 }}>
                                SRIBEESonline
                            </h1>
                        ...
                        <div style={{ textAlign: 'right', fontSize: 12, color: '#666', lineHeight: 1.6 }}>
                            <div>
                                <strong>Period:</strong> {days} Days ({summary ? `${dayjs(summary.range.start_date).format('MMM D, YYYY')} – ${dayjs(summary.range.end_date).format('MMM D, YYYY')}` : ''})
                            </div>
                            <div>
                                <strong>Scope:</strong> {branchId ? (branchesQuery.data?.branches.find(b => b.branch_id === branchId)?.name || 'Branch Scoped') : 'All Branches'}
                            </div>
                            <div>
                                <strong>Generated By:</strong> {user?.full_name || 'System Administrator'}
                            </div>
                            <div>
                    ...
                    <div style={{ marginBottom: 24 }}>
                        <h3 style={{ margin: '0 0 12px 0', color: '#111', fontSize: 15, borderLeft: '4px solid #1890ff', paddingLeft: 8 }}>
                            1. Key Performance Indicators
                        </h3>
                        <Row gutter={[12, 12]}>
                            <Col span={8}>
                                <div style={{ background: '#fafafa', padding: 12, borderRadius: 6, border: '1px solid #eee' }}>
                                    <Text type="secondary" style={{ fontSize: 12 }}>Total Revenue</Text>
                                    <div style={{ fontSize: 18, fontWeight: 'bold', color: '#1890ff' }}>{formatLKR(current?.revenue)}</div>
                                </div>
                            </Col>
                            <Col span={8}>
                                <div style={{ background: '#fafafa', padding: 12, borderRadius: 6, border: '1px solid #eee' }}>
                                    <Text type="secondary" style={{ fontSize: 12 }}>Total Orders</Text>
                                    <div style={{ fontSize: 18, fontWeight: 'bold' }}>{formatNumber(current?.orders)} ({current?.paid_orders ?? 0} paid)</div>
                                </div>
                            </Col>
                            <Col span={8}>
                                <div style={{ background: '#fafafa', padding: 12, borderRadius: 6, border: '1px solid #eee' }}>
                                    <Text type="secondary" style={{ fontSize: 12 }}>Average Order Value</Text>
                                    <div style={{ fontSize: 18, fontWeight: 'bold' }}>{formatLKR(current?.avg_order_value)}</div>
                                </div>
                            </Col>
                            <Col span={12}>
                                <div style={{ background: '#fafafa', padding: 12, borderRadius: 6, border: '1px solid #eee' }}>
                                    <Text type="secondary" style={{ fontSize: 12 }}>Active Customers</Text>
                                    <div style={{ fontSize: 18, fontWeight: 'bold' }}>{formatNumber(current?.customers)} ({summary?.new_customers ?? 0} first-time)</div>
                                </div>
                            </Col>
                            <Col span={12}>
                                <div style={{ background: '#fafafa', padding: 12, borderRadius: 6, border: '1px solid #eee' }}>
                                    <Text type="secondary" style={{ fontSize: 12 }}>Total Items Sold</Text>
                                    <div style={{ fontSize: 18, fontWeight: 'bold' }}>{formatNumber(current?.items_sold)}</div>
                                </div>
                            </Col>
                        </Row>
                    </div>
                    ...
                    <div style={{ marginBottom: 24 }}>
                        <h3 style={{ margin: '0 0 12px 0', color: '#111', fontSize: 15, borderLeft: '4px solid #1890ff', paddingLeft: 8 }}>
                            2. Order Fulfilment &amp; Status Breakdown
                        </h3>
                        <Table
                            size="small"
                            pagination={false}
                            ...
                            dataSource={statusRows}
                            columns={[
                                { title: 'Status', dataIndex: 'status', render: (s: string) => titleCase(s) },
                                { title: 'Orders', dataIndex: 'orders', align: 'right', render: formatNumber },
                                {
                                    title: 'Share',
                                    align: 'right',
                                    render: (_, r) => (statusTotal ? ((r.orders / statusTotal) * 100).toFixed(1) + '%' : '0%'),
                                },
                                { title: 'Revenue', dataIndex: 'revenue', align: 'right', render: (v: number) => formatLKR(v) },
                            ]}
                        />
                    </div>
                    ...
                    <div style={{ marginBottom: 24 }}>
                        ...
                        <Table
                            ...
                            pagination={false}
                            rowKey="category_id"
                            dataSource={categorySlices}
                            columns={[
                                { title: 'Category Name', dataIndex: 'name' },
                                { title: 'Units Sold', dataIndex: 'units_sold', align: 'right', render: formatNumber },
                                { title: 'Market Share', dataIndex: 'share', align: 'right', render: (v: number) => `${v}%` },
                                { title: 'Total Revenue', dataIndex: 'revenue', align: 'right', render: (v: number) => formatLKR(v) },
                            ]}
                        />
                    </div>
                    ...
                    <div style={{ marginBottom: 24 }}>
                        ...
                        <Table<TopProduct>
                            ...
                            pagination={false}
                            rowKey="product_id"
                            dataSource={(productsQuery.data || []).slice(0, 5)}
                            columns={[
                                { title: '#', render: (_, __, i) => i + 1, width: 40 },
                                { title: 'Product Name', dataIndex: 'name' },
                                { title: 'Category', dataIndex: 'category', render: (v) => v || '—' },
                                { title: 'Units Sold', dataIndex: 'units_sold', align: 'right', render: formatNumber },
                                { title: 'Revenue', dataIndex: 'revenue', align: 'right', render: (v) => formatLKR(v) },
                            ]}
                        />
                    </div>
                    ...
                    <div style={{ marginBottom: 16 }}>
                        ...
                        <Table<TopCustomer>
                            ...
                            pagination={false}
                            rowKey="user_id"
                            dataSource={(customersQuery.data || []).slice(0, 5)}
                            columns={[
                                { title: '#', render: (_, __, i) => i + 1, width: 40 },
                                { title: 'Customer Name', dataIndex: 'full_name' },
                                { title: 'Contact', render: (_, r) => r.email || r.phone || '—' },
                                { title: 'Orders', dataIndex: 'orders', align: 'right', render: formatNumber },
                                { title: 'Total Spent', dataIndex: 'revenue', align: 'right', render: (v) => formatLKR(v) },
                            ]}
                        />
                    </div>

                    {/* Document Footer */}
                    <div style={{ marginTop: 24, paddingTop: 12, borderTop: '1px solid #eee', textAlign: 'center', fontSize: 11, color: '#888' }}>
                        Confidential — SRIBEESonline Internal Executive Management Document
                    </div>
                </div>
            </Modal>
        </div>
